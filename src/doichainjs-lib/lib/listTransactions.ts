...
export async function listTransactions(address: any, o_options: any, addressList: any) {
    ...
    //if this is a p2pkh
    let script = bitcoin.address.toOutputScript(address, global.network)  
    ...
    try {
        ...
        const history = await global.blockchain_scripthash_getHistory(
            reversedHash.toString("hex")
        )
        
        const UTXO = await global.blockchain_scripthash_listunspent(
            reversedHash.toString("hex")
        )
        ...
        for (const tx of UTXO) {
            ...
            const decryptedTx = await global.blockchain_transaction_get(transaction.tx_hash, 1)
        ...
        console.info('history length',result.length)
            ...
            const balance = await global.blockchain_scripthash_getBalance(
              reversedHash.toString("hex")
            );
            console.log("Balance: ", balance);
            
            const UTXOs = await global.blockchain_scripthash_listunspent(
              reversedHash.toString("hex")
            );
